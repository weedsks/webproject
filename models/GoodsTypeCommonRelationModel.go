...
func (m *GoodsTypeCommonRelationModel) TableName() string {
	return TableNmae("goods_typeCommon_relation")
}
...
func (m *GoodsTypeCommonRelationModel) GetRelation(goodsId int) (lists map[int]TypeCommonModel, err error) {
	o := orm.NewOrm()
	sql_list := "select t.id,t.name from ks_goods_typeCommon_relation as r left join ks_type_common as t on t.id=r.type_common_id where r.goods_id = ?"
