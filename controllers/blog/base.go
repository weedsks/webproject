...
//返回静态文件模块目录
func (p *baseController) getHomeFix() string {
	return beego.AppConfig.String("homefix")
}
...
func (p *baseController) Prepare() {
	...
	p.Data["GoodsTypeDesc"] = models.GoodsTypeDesc()

	p.Layout = p.getHomeFix() + "/common/layout.html"
	p.LayoutSections = make(map[string]string)
	p.LayoutSections["HtmlHead"] = p.getHomeFix() + "/common/head.html"
	p.LayoutSections["HtmlFoot"] = p.getHomeFix() + "/common/foot.html"
}
